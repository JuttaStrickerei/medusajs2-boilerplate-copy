...
Create these tasks when implementing workflows:
...
### Loops
- ❌ No `for`/`while` loops → Use alternatives based on your use case
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  for (const item of items) {
    await myWorkflow(req.scope).run({ item })
  }
...
**Alternative 2: Use `transform` for Array Operations**
...
**⚠️ IMPORTANT**: Before creating custom steps, check if Medusa provides a built-in step for your use case.
