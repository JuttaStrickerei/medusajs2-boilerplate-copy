...
function getActiveFulfillment(order: OpenOrder): OrderFulfillment | null {
  const active = (order.fulfillments || [])
    ...
    .sort(
      (a, b) =>
        // newest first — we operate on the latest active fulfillment
      ...
      {orders.map((order) => {
        ...
        const isPending =
          ...
          (markPickedUpMutation.isPending &&
            markPickedUpMutation.variables?.orderId === order.id)
          ...
          >
            ...
            {isReady && activeFf ? (
              <Button
                ...
                disabled={isPending}
                isLoading={isPending}
                onClick={() => handleMarkPickedUp(order.id, activeFf.id)}
                className="shrink-0"
              >
                {isPending
