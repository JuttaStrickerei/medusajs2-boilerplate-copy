...
export const PICKUP_READY = 'pickup-ready'

interface PickupReadyPreviewProps {
  order: OrderDTO & { display_id: string; summary: { raw_current_order_total: { value: number } } }
  pickupLocation?: string
  companyLogo?: string
}

export interface PickupReadyTemplateProps {
  order: OrderDTO & { display_id: string; summary: { raw_current_order_total: { value: number } } }
  pickupLocation?: string
  companyLogo?: string
  preview?: string
...
export const isPickupReadyTemplateData = (data: any): data is PickupReadyTemplateProps =>
  typeof data?.order === 'object'

export const PickupReadyTemplate: React.FC<PickupReadyTemplateProps> & {
  PreviewProps: PickupReadyPreviewProps
} = ({ order, pickupLocation, companyLogo, preview = 'Ihre Bestellung wartet im Shop auf Sie.' }) => {
  return (
        ...
        <Container style={{ maxWidth: '600px' }}>
            ...
            <tr>
              <td align="center" style={{ paddingBottom: '24px', borderBottom: '1px solid #e7e5e4' }}>
                ...
                <Text style={{
                  ...
                  Ihre Bestellung ist zur Abholung bereit
                </Text>
                <Text style={{
                  fontSize: '14px',
                  color: '#57534e',
          ...
          {pickupLocation && (
            <table width="100%" cellPadding="0" cellSpacing="0" style={{ marginBottom: '24px', paddingTop: '24px', borderTop: '1px solid #e7e5e4' }}>
              <tr>
                <td>
                  <Text style={{ fontSize: '14px', fontWeight: '500', color: '#1c1917', margin: '0 0 8px 0' }}>
                    Abholort
                  </Text>
                  {pickupLocation.split('\n').map((line, idx) => (
                    <Text key={idx} style={{ fontSize: '14px', color: '#57534e', margin: '0 0 2px 0', lineHeight: '20px' }}>
                      {line}
                    </Text>
                  ))}
                </td>
            ...
            <tr>
              <td align="center">
                <Text style={{ fontSize: '13px', color: '#57534e', margin: '0 0 8px 0', lineHeight: '20px' }}>
                  Vielen Dank für Ihre Bestellung! Bei Fragen erreichen Sie uns unter{' '}
...
PickupReadyTemplate.PreviewProps = {
  order: {
    id: 'test-order-id',
    display_id: 'ORD-123',
  ...
  pickupLocation: 'Strickerei Jutta\nMusterstraße 1\n1234 Musterstadt\nÖsterreich',
} as PickupReadyPreviewProps

export default PickupReadyTemplate
